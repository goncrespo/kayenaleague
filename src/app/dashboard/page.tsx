...
export default async function DashboardPage() {
  ...
  type MatchItem = { id: string; roundNumber: number; deadlineDate: Date; player1: { name: string | null }; player2: { name: string | null } };
          ...
          <ul className="space-y-2">
            {upcomingMatches.map((m: MatchItem) => (
              <li key={m.id} className="flex justify-between border rounded-md p-3">
                <span>Jornada {m.roundNumber}: {m.player1.name ?? ""} vs {m.player2.name ?? ""}</span>
